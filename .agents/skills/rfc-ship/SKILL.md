...
- **用户说「把 rfc 都做了」「实施待办」** → 默认**全部待办**，整合进**一个** PR。此时逐篇只读其 `## 提案`、`## 明确不在本次范围`、`## 验收标准` 三段，**不要连 `docs/rfc/archive/` 或 `docs/archive/` 一起读**——已实施与历史笔记对实施没有指导作用（`docs/archive/` 实测 23 篇 173 KB ≈ 5 万 token，约等于把整个 `src/` 再读一遍）。
