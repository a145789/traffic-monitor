...
把看门狗窗口提升为唯一的稳定控制消息入口，生产消费者与非生产消费者划分如下：生产消费者为 `--quit` 流程（`src/main.rs:92`）、更新工作线程的退出转发（`src/update/mod.rs:562` 的 `scan_subprocess_protocol` 回调）、`TaskbarCreated` 重建（`src/main.rs:376`）；非生产消费者为 `scan_subprocess_protocol` 的 Cursor 协议单测（仅覆盖文本解析，不覆盖 HWND 生命周期）。具体改动：`quit_existing_instance` 改查 `WATCHDOG_CLASS`（顶层，`FindWindowW` 可达），看门狗过程新增退出分支（收到退出命令后转发给 `CURRENT_MAIN_HWND` 若有效，否则直接走与 `src/main.rs:501` 等价的托盘清理加 `PostQuitMessage`，且该操作幂等）；`spawn_update_worker` 改为携带看门狗 HWND 或由工作线程 `PostMessageW` 给看门狗，再由看门狗按当前 `CURRENT_MAIN_HWND` 转发或直接执行 `handle_update_action` 语义，同时把“读到 EXIT_MAIN”与“成功通知 UI”拆成两个返回值/标志，转发失败时工作线程必须复位 `UPDATE_IN_PROGRESS` 而不是按 `exit_signalled` 直接返回；`rebuild_main_window` 的创建失败路径改为由看门狗侧带退避的重试（定时器或计数器驱动），而不是一次性 `show_error` 后丢空窗口状态。
