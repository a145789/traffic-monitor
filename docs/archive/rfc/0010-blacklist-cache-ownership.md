...
- 重构最易破坏「失败沿用旧表一个缓存周期」语义：验收时须专门确认失败路径不会变成每秒重试 `GetAdaptersAddresses`（只有首次调用返回 `ERROR_BUFFER_OVERFLOW` 后才会执行第二次填充调用，其余错误直接返回 `None`、不进行第二次调用；高频重试主要放大「探测 + 填充」两次调用的固定开销）。
