...
1. **退出幂等门只覆盖三条入口中的一条，且 claim 悬在调用链中间。** `src/main.rs:71-73` 写明「退出序列（托盘清理 + `PostQuitMessage`）只应执行一次」，但 `claim_exit_request`（`src/main.rs:470-472`）在生产路径上只被 `route_exit_request`（`src/main.rs:528-533`）调用；而这条路径是全仓唯一的三层调用链——`WM_USER_QUIT_REQUEST`（`src/main.rs:571-572`）→ `route_exit_request` 先 claim → `finish_exit_from_watchdog`（`src/main.rs:517-520`，负责 `disarm_rebuild_retry`）→ `begin_exit`（`src/main.rs:505-512`）。另外两条入口完全不过门：`WM_CLOSE`（`src/main.rs:737-740`）直接调 `begin_exit`，`handle_update_action`（`src/update/mod.rs:806-820`）直接执行收尾。并发到达时清理与 `PostQuitMessage` 会执行两遍——两操作本身幂等、无实际危害，但「只执行一次」这条不变量在文档与实现之间已不成立；且 claim 悬在链中间，使「把门收到 `begin_exit`」这一步极易写成「两处 claim 并存」（后果见提案 1）。
2. **退出收尾有两份实现。** `begin_exit`（`src/main.rs:505-512`）的注释写着「两个调用方…共用这一份实现，避免退出语义在两处漂移」，实际是全仓 `PostQuitMessage` 的非导入、非注释命中恰好两处：`src/main.rs:510` 与 `src/update/mod.rs:818`；`remove_tray_icon` 的退出收尾调用同样落在这两处（全仓非导入、非定义命中另有第三处 `src/main.rs:446`，属 Explorer 重建清理，不在退出收尾内）。
...
另有**两处测试触碰进程级全局**：`src/main.rs:832` 的 `CURRENT_MAIN_HWND.store_raw` 与 `src/update/mod.rs:1086` 的 `UPDATE_IN_PROGRESS`；其中 `src/update/mod.rs` 侧的用例注释已自知并声明「只保留这一处」，`src/main.rs` 侧无对应声明；两者在 `cargo test` 并行执行下仍是地雷。

## 提案

1. **把 claim 迁移到 `begin_exit`（收敛为唯一门），而不是新增一处。** 具体做法：`begin_exit` 开头 `if !claim_exit_request(&EXIT_REQUESTED) { return; }`，**同时删除 `route_exit_request` 里现有的那句 `claim_exit_request`**，使该函数退化为「`finish_exit_from_watchdog`（`disarm_rebuild_retry` + `begin_exit`）」。⚠️ 若不删旧 claim，就会形成两处 claim 并存：`--quit` 路径已在 `route_exit_request` 置位，`begin_exit` 的第二次 `swap` 必返回 `true` → 提前 return → **托盘不清理、`PostQuitMessage` 不投递、进程根本不退出**，子进程只能白等 5 秒后落到安装器 taskkill 兜底。本项把「只执行一次」从注释承诺变成代码事实；`src/main.rs:522-527` 的论证（「直接执行让幂等门恰好对应『退出序列已执行』这一不可逆事实」）随之移写到 `begin_exit` 上方。
2. `handle_update_action`（`src/update/mod.rs:806-820`）收敛为两步：`UPDATE_IN_PROGRESS.store(false, Ordering::Release)` + 调 `crate::begin_exit()`（可见性提到 `pub(crate)`），删掉与 `begin_exit` 逐行重复的 `remove_tray_icon` 与 `PostQuitMessage` 尾段。该路径不触碰 `EXIT_REQUESTED`，因此 `begin_exit` 的门会正常放行；`src/update/mod.rs` 已在反向引用 `crate::util`（`crate::tray` 的引用随本项删掉的尾段一并消失），不引入新的依赖方向类型。本项须排在提案 1 之后。
...
5. （文档）`AGENTS.md` 增第 10 条：「新增跨消息/跨线程事实时，必须指认唯一真值源并写明竞争处理」。现有 6 个真值源（`EMBEDDED`、`EXIT_REQUESTED`、`UPDATE_IN_PROGRESS`、`LAST_CHECK_TIME`、`SUSPEND_REASONS`、`LAST_RENDERED_VALUES`）已按此风格收口，这条是把既有隐性惯例显性化，符合 AGENTS 自己的更新指南（引入新的隐式设计约束）。
...
- 不去解耦测试对进程级全局的触碰（`src/main.rs:832` 的 `CURRENT_MAIN_HWND.store_raw`、`src/update/mod.rs:1086` 的 `UPDATE_IN_PROGRESS`）：收益小、需重构测试骨架，且这两个全局的读本身是设计的一部分。改注释即止。
...
- `grep -n 'PostQuitMessage' src/` 的非导入、非注释命中只剩 `begin_exit` 一处；`grep -n 'remove_tray_icon' src/` 的非导入、非定义命中只剩 `begin_exit`、`rebuild_main_window` 的重建清理（`src/main.rs:446`）与启动失败早退清理（`src/main.rs:255-258`）三处，`src/update/mod.rs` 不再出现该符号。
- `grep -n 'claim_exit_request' src/` 在生产代码的命中恰好两处，且都在 `src/main.rs`：定义（`src/main.rs:470`）与 `begin_exit` 内的**唯一**调用（`exit_request_gate_accepts_only_first_request` 用例内的三处调用属「原样保留」范围，不计）；`route_exit_request` **不得**再出现该调用——出现即说明落成了「两处 claim 并存」的错误实现。
...
- `begin_exit` 过门后存在理论窗口：「已置位但 `PostQuitMessage` 尚未生效」期间到达的新请求会被吞掉。既有论证（`src/main.rs:522-527`）认为该窗口不存在（置位即序列已开始、退出不可逆），实施时须把同一论证落到 `begin_exit` 注释，否则后人无法审计这条取舍。
