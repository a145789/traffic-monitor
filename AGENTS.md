...
- Rust edition 2024，依赖 `windows` crate v0.62（含 WinHTTP、BCrypt、RemoteDesktop feature）和 `hidapi` v2.6
...
```bash
bun scripts/release.ts 0.5.1    # 更新版本号 → 编译 → git tag → GitHub Release（CI 自动打包安装包）
bun scripts/package.ts          # 本地构建编译 + 打包安装包（版本号取自 Cargo.toml）
bun scripts/package.ts dev  # 补丁版本号（如 0.3.4-devxxxxxx）后构建打包
```
...
| `update.rs`    | 自动更新检查、安装包下载、SHA-256 校验、UAC 提权安装                          |
...
- 退避：连续 5 次零速且无活跃接口触发退避（`BACKOFF_ZERO_THRESHOLD=5`），退避间隔 `TIMER_INTERVAL_NETWORK_BACKOFF=15000`
...
- 菜单：`MENU_ID_SHOW_MOUSE=1003`, `MENU_ID_RESTART_HID=1004`, `MENU_ID_AUTO_UPDATE_TOGGLE=1005`, `MENU_ID_CHECK_UPDATE_MANUAL=1006`
- 原子变量：`CPU_USAGE`/`MEM_USAGE`/`NET_SPEED_UP`/`NET_SPEED_DOWN`（AtomicU32），`MOUSE_BATTERY_LEVEL`/`MOUSE_DPI_VALUE`（AtomicU32），`MOUSE_ONLINE`/`SUSPENDED`/`FULLSCREEN`/`SHOW_MOUSE_INFO`/`MOUSE_IS_CHARGING`/`ENABLE_AUTO_UPDATE`/`UPDATE_IN_PROGRESS`（AtomicBool），`NETWORK_BACKOFF`/`CONSECUTIVE_ZERO_COUNT`（AtomicBool/AtomicU32）
- 网络消息（定义在 `collector.rs`）：`WM_USER_NETWORK_DISCONNECTED`（`WM_USER + 3`）、`WM_USER_NETWORK_RECONNECTED`（`WM_USER + 4`）
...
菜单项：版本号展示（0，禁用状态）、分隔线、开机自启（1001）、自动检查更新（1005，可勾选）、检查更新...（1006）、显示鼠标信息（1003）、重置鼠标（1004，仅鼠标显示时可见）、退出（1002）。右键菜单用 `InsertMenuItemW` 创建。托盘回调消息为 `WM_APP_TRAY`（`WM_USER + 100`），使用 `NOTIFYICON_VERSION_4`。版本号从 `Cargo.toml` 编译时获取（`env!("CARGO_PKG_VERSION")`）。
...
- 全屏：`check_fullscreen` 在 `TIMER_ID_NETWORK` 回调中调用，前台窗口尺寸 == 所在显示器分辨率（`GetForegroundWindow` + `GetWindowRect` vs `MonitorFromWindow` + `GetMonitorInfoW`），排除桌面和 Shell 窗口，支持多显示器
- 睡眠：`PBT_APMSUSPEND`
...
- `HidApi` 实例在线程循环内按需创建（`HidApi::new()`），用完即丢。设计上不缓存 HidApi 实例：轮询间隔 180s/300s，重建开销可忽略，且避免常驻内存。不要提议将 HidApi 提升为长生命周期缓存。
...
- 消息：`WM_USER_UPDATE_READY = WM_USER + 5`，状态码 `UPDATE_STATUS_NO_UPDATE`(0) / `UPDATE_STATUS_PORTABLE_FOUND`(1) / `UPDATE_STATUS_INSTALLED_READY`(2) / `UPDATE_STATUS_ERROR`(3)
- 自动检查：启动时 + 网络恢复时调用 `start_auto_check(hwnd)`，冷却 1 小时（失败时 5 分钟），通过 `ENABLE_AUTO_UPDATE` 原子变量控制开关
- 手动检查：`start_manual_check(hwnd)`，无冷却限制，结果均弹窗反馈
- 版本检测：从 `github.com/a145789/traffic-monitor/releases/latest/download/version.txt` 获取版本号 + SHA-256 哈希
- 安装版流程：下载安装包到 `%LOCALAPPDATA%\Traffic Monitor\traffic-monitor-setup-temp.exe`，SHA-256 校验后弹窗确认，用户同意后 `ShellExecuteExW` 以 `runas` 提权安装并退出
- 免安装版流程：发现新版本时提示打开 GitHub Release 页面手动下载
- 下载回退：主域名 `github.com` 失败后回退到 `ghproxy.cn` 代理
- RAII 守卫：`WinHttpHandles`（WinHTTP 句柄）、`BcryptHandles`（BCrypt 哈希句柄）
- 注册表：`HKCU\Software\Traffic Monitor\EnableAutoUpdate`（DWORD）持久化自动更新开关
- `init_cleanup_temp()` 启动时清理残留临时文件
- `load_auto_update_enabled()` 启动时从注册表加载开关状态
...
- 断网退避：连续 5 次零速且无活跃接口触发退避（`NETWORK_BACKOFF`），collector 通过 `PostMessageW` 发送 `WM_USER_NETWORK_DISCONNECTED`/`WM_USER_NETWORK_RECONNECTED` 到主窗口，wnd_proc 据此切换定时器间隔（1000ms ↔ 15000ms）
