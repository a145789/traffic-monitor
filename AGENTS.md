...
菜单项：版本号展示（0，禁用状态）、分隔线、开机自启（1001）、显示鼠标信息（1003）、重置鼠标（1004，仅鼠标显示时可见）、退出（1002）。右键菜单用 `InsertMenuItemW` 创建。托盘回调消息为 `WM_APP_TRAY`（`WM_USER + 100`），使用 `NOTIFYICON_VERSION_4`。版本号从 `Cargo.toml` 编译时获取（`env!("CARGO_PKG_VERSION")`）。
