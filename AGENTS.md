...
cargo fmt                       # 格式化代码
```
*注：如果修改涉及到 `unsafe` 代码，必须严格符合 [docs/unsafe-code-policy.md](docs/unsafe-code-policy.md) 中规定的安全要求。上述构建、Clippy 和格式化校验仅在修改了 Rust 相关的源码文件时才需要执行。*
...
```
*注：版本号需同步更新 [Cargo.toml](Cargo.toml) 和 [installer.iss](installer.iss)，更新规范见 `VERSIONING.md`。*
