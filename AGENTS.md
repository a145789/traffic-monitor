...
1. **鼠标 HID 轮询与 `HidApi` 实例化**
   ...
   - **防坑**：`SKIP_WARMUP.store(true)` 必须在 `restart_mouse_thread()` **之前**调用，因为 `start_mouse_thread()` 内部通过 `swap(false)` 原子读取并消费该标志。调换顺序会导致标志被忽略。
   - **设计决策**：`poll_mouse()` 中电量和 DPI 查询**独立失败、互不影响**。当有线设备（`VID_WIRED`）返回 `charging=true && level>=100` 时，自动尝试从 2.4G 设备（`VID_WIRELESS`）获取真实电量。此 fallback 依赖 `MOUSE_VIDS` 数组中 VID 的声明顺序（有线优先）。
2. **窗口嵌入任务栏的顺序**
